// src/components/tabs/EditMediaTab.tsx
"use client";
...
type Props = {
  ...
  videoUrl?: string | null; // mevcut (remote) video url
  videoFile?: File | null; // seçilmiş (local) video
  onRemoveVideo?: () => void; // videoyu kaldır
  videoAccept?: string; // default: "video/*"
};

function isRemote(item: MediaItem): item is { id: string; kind: "remote"; url: string } {
  return item.kind === "remote";
}

function isLocal(item: MediaItem): item is { id: string; kind: "local"; file: File } {
...
export default function EditMediaTab({
  ...
  // Cleanup orphaned local image URLs
  useEffect(() => {
  ...
  // Cleanup all local image URLs on unmount
  useEffect(() => {
    return () => {
  ...
  const closeModal = useCallback(() => setOpenSortModal(false), []);

  // Escape closes modal
  useEffect(() => {
    if (!openSortModal) return;
  ...
  const effectiveVideoSrc = localVideoPreview || videoUrl || null;
  const showVideoBlock = Boolean(onPickVideo || effectiveVideoSrc || onRemoveVideo);
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            title={
              mediaItems.length < 2 ? "Sıralamak için en az 2 fotoğraf olmalı" : "Fotoğrafları sırala"
            }
          >
            <ListOrdered size={18} />
            Sırala
          ...
          <label
            ...
            title={remaining === 0 ? "Maksimum fotoğraf sayısına ulaşıldı" : "Fotoğraf ekle"}
          >
            <ImagePlus size={18} />
            Fotoğraf Ekle
            <input
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              ...
              <div className="mt-1 text-xs text-gray-500">
                İstersen ilan videosu ekleyebilirsin. Büyük dosyalar upload süresini uzatır, evet şok edici.
              </div>
              {videoFile && (
                <div className="mt-2 text-xs text-gray-700">
                  Seçili: <span className="font-semibold">{videoFile.name}</span> • {formatMB(videoFile.size)}
                </div>
              )}
              {!videoFile && videoUrl && (
                <div className="mt-2 text-xs text-gray-700">
                  Mevcut video: <span className="font-semibold">Yayında</span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-2">
              {onPickVideo && (
                <label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-semibold text-gray-900 hover:bg-gray-50 transition">
                  <Video size={18} />
                  Video Seç
                  <input
                    type="file"
              ...
              {onRemoveVideo && (videoFile || videoUrl) && (
                <button
                  type="button"
                  onClick={() => {
                    const ok = confirm("Video kaldırılacak. Devam edilsin mi?");
                    if (!ok) return;
                    onRemoveVideo();
                  }}
                  className="inline-flex items-center justify-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm font-semibold text-red-700 hover:bg-red-100 transition"
      ...
      {mediaItems.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-300 bg-white p-10 text-center">
          <div className="text-sm font-semibold text-gray-900">Henüz fotoğraf yok</div>
          <div className="mt-1 text-xs text-gray-500">“Fotoğraf Ekle” ile görselleri yükleyebilirsin.</div>
        </div>
      ) : (
        <>
          ...
          <div className="fixed inset-0 z-[80]">
            ...
            <div className="fixed inset-0 flex items-center justify-center p-4">
              <motion.div
                ...
                role="dialog"
                aria-modal="true"
                aria-label="Resim Sıralama"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="p-6 border-b border-gray-200 bg-white rounded-t-2xl">
                  <div className="flex items-center justify-between">
                    <div>
                      <h2 className="text-xl font-bold text-gray-900">Resim Sıralama</h2>
                      <p className="text-sm text-gray-600 mt-1">
                        {mediaItems.length} adet resim • İlk resim kapak olarak kullanılacaktır
                      </p>
                    </div>
                    <button
                      ref={closeBtnRef}
                ...
                <div className="flex-1 overflow-y-auto p-6">
                  <p className="text-sm text-gray-600 mb-6">
                    Sıralamak için ↑ / ↓ butonlarını kullan. Kapak fotoğrafı: <b>1. sıradaki</b>.
                  </p>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-gray-500">
                              {isLocal(item)
                                ? `${formatMB(item.file.size)} • ${item.file.type.split("/")[1] || ""}`
                                : "Yayındaki görsel"}
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className={`p-2 rounded-lg transition-colors ${
                                index === mediaItems.length - 1 ? "opacity-30 cursor-not-allowed" : "hover:bg-gray-100"
                              }`}
                              aria-label="Aşağı al"
                              title="Aşağı al"
                            >
                            ...
                            <div className="p-2 text-gray-400 rounded-lg" aria-hidden="true" title="Sürükle-bırak (şimdilik görsel)">
