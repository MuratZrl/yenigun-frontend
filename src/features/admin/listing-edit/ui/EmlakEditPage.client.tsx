// src/features/admin/emlak-edit/ui/EmlakEditPage.client.tsx
"use client";
...
import {
  ...
  directionOptions, heatingOptions, keyOptions,
  yesNoOptions, zoningStatusOptions,
} from "@/data/propertyData";

import { useEmlakEditController } from "../hooks/useEmlakEditController";

export default function EmlakEditPage() {
  const c = useEmlakEditController();
      ...
      case 2:
        return (
          <EditAdDetailsTab
            ...
            setFeatureValues={c.setFeatureValues}
            ...
            setMediaItems={c.setMediaItems}
            onPickImages={c.handleImageChange}
            onRemoveMedia={c.removeMediaItem}
            videoFile={c.videoFile}
            setVideoFile={c.setVideoFile}
            onPickVideo={c.handleVideoChange}
            existingVideoUrl={c.existingVideoUrl}
        ...
        <div className="max-w-5xl mx-auto">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-gray-600 text-sm lg:text-base truncate">
                  {breadcrumb.length > 0 ? `Emlak > ${breadcrumb.join(" > ")}` : "İlan bilgilerini düzenleyin"}
                </p>
              </div>
              <Link href="/admin/emlak" className="bg-custom-orange text-white px-4 py-2 rounded-lg text-sm flex items-center gap-2 w-fit shrink-0">
                <ArrowLeft size={18} />
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="bg-gray-100 h-1.5 w-full">
              <motion.div className="h-full bg-blue-500" initial={{ width: 0 }} animate={{ width: `${(c.activeTab / totalSteps) * 100}%` }} transition={{ duration: 0.5 }} />
            </div>
